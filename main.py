...
# الرقم الموحد لإنستا باي وفودافون كاش
PAYMENT_NUMBER = os.getenv("INSTAPAY_HANDLE", "01060746538")

logger.info("=" * 60)
logger.info("🚀 KIDS STORY BOT v6.5 - MULTI-PAYMENT OPTIONS 🚀")
logger.info("=" * 60)
...
def start_processing(sender_id, messaging_event, background_tasks):
    ...
    if "quick_reply" in message:
        ...
        if current_step == "waiting_for_gender":
            user_state[sender_id]["gender"] = payload
...
def process_story_generation(sender_id, value, is_preview=False):
    try:
        ...
        if is_preview:
            cover_ai_url = generate_storybook_page(char_desc, f"Watercolor cover, {value}", is_cover=True)
            if cover_ai_url:
                ...
                # --- رسالة الدفع المحدثة ---
                msg = (
                    f"💰 لإكمال قصة {child_name} الرائعة، يرجى تحويل مبلغ 25 جنيه عبر إحدى الطرق التالية:\n\n"
                    f"1️⃣ انستا باي (InstaPay) على الحساب:\n✨ {PAYMENT_NUMBER} ✨\n\n"
                    f"2️⃣ فودافون كاش (Vodafone Cash) على الرقم:\n✨ {PAYMENT_NUMBER} ✨\n\n"
                    f"📸 بعد التحويل، أرسلي صورة التحويل (Screenshot) هنا فوراً لتفعيل القصة! 👇"
                )
        ...
        for i, p in enumerate(pages_prompts):
            ...
            img_url = generate_storybook_page("", p["prompt"])
            if img_url:
